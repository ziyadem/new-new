...
const ProductCard = ({ res }: any) => {
  ...
  return (
    ...
    >
      ...
      >
        {res?.images?.map(({ image }: { image: string }) => (
      ...
      <Box className="w-[50%] max-[1500px]:w-full">
        <Box className="flex items-center gap-10">
          <Title className="text-[1.2em] capitalize">
            {res && res["name_" + localStorage.getItem("i18nextLng")]}
          </Title>
        </Box>
        <Space />
        ...
        <Description>
          {res && res["description_" + localStorage.getItem("i18nextLng")]}
        </Description>
        <Space />
        {res?.characteristic?.length > 0 ? (
          <Section>
            {res?.characteristic?.map(
