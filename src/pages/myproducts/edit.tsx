...
const MyProductEdit = () => {
  ...
  useEffect(() => {
    dispatch(main({ title: "my_product_edit_title" }));
  }, []);
  ...
  const { res, loader } = useFetch(
    `/marketplace/dashboard/shop/${data?.shop}/product/${id}/`,
    1
  );
  ...
  useEffect(() => {
    res?.product?.variants?.length > 0
      ? setData((prev) => ({
          ...prev,
          ...res?.product,
          variants: res?.product?.variants?.map((e: CustomVariant) => ({
            ...e,
            variant: e?.guid,
          })),
        }))
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = `/marketplace/dashboard/shop/update-product/`;
