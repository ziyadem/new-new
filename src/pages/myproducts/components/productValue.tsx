  ...
  return (
    ...
    >
      <Box className="w-full grid grid-cols-4 gap-3 max-[850px]:grid-cols-2 max-[500px]:grid-cols-1">
        ...
        <TextField
          fullWidth
          disabled={Boolean(res?.product?.variants?.length)}
          size="small"
          label={t("all_count")}
          value={res?.product?.variants?.length > 0 ? all_count : data?.count}
          onChange={(
              ...
              <InputAdornment position="end">
                <IconButton
                  ...
                  onClick={() => {
                    setData((prev: any) => ({
                      ...prev,
                      sku: generateSku(data?.sku || "", res?.product?.name_uz),
                    }));
                  }}
                >
                  <TfiReload />
      ...
      {data?.variants?.length > 0 ? (
        ...
        >
          <DataGrid
            ...
            components={{ NoRowsOverlay: Empty, Pagination: null }}
          />
        </Section>
      ) : null}
    </Card>
